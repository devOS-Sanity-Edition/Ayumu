...
uniform vec4 color_old7;//r,g,b of color3 to replace, tolerance between 0 and 1.
uniform vec3 color_new7;//r,g,b of the new color7.

uniform vec4 color_old8;//r,g,b of color4 to replace, tolerance between 0 and 1.
uniform vec3 color_new8;//r,g,b of the new color8.
...
void main()
{
	...
	//Find the color difference between the sample and the color_old7.
    diff = dot(abs(sample - color_old7).rgb, vec3(1. / 3.));
    //Set the color_new7 when the the difference is greater than the tolerence.
    color = mix(color, color_new7, step(diff, color_old7.w));
	
	//Find the color difference between the sample and the color_old8.
    diff = dot(abs(sample - color_old8).rgb, vec3(1. / 3.));
    //Set the color_new8 when the the difference is greater than the tolerence.
    color = mix(color, color_new8, step(diff, color_old8.w));
